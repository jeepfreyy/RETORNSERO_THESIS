...
def list_incidents():
    ...
    return jsonify([{
        ...
        'threat_level': i.threat_level or 5,
        'clip_filename': i.clip_filename,
        'duration': i.duration,
        'incident_status': i.incident_status or 'OPEN',
        'reporter_name': i.reporter_name or 'Unknown',
        'resolution_note': i.resolution_note or '',
        'people_count': i.people_count or 0,
        'location': i.location or '',
        'thumbnail_url': (
            '/archive_media/' + os.path.relpath(
                i.clip_path.replace('.mp4', '.jpg'), ARCHIVE_DIR
            ).replace('\\', '/')
            if i.clip_path and os.path.exists(i.clip_path.replace('.mp4', '.jpg'))
            else ''
        ),
        'responders': [{
            'id': r.id,
            'name': r.responder_name,
            'role': r.responder_role,
...
def save_incident():
    ...
    try:
        ...
        title = bleach.clean(data.get('title', '') or '', tags=[], strip=True)[:200]
        status = data.get('status', 'VALID_THREAT')
        density_tag = data.get('density_tag', 'LOW')
        camera_id = data.get('camera_id', 'CAM-01')
        duration = data.get('duration', 0)
        try:
            threat_level = max(1, min(10, int(data.get('threat_level', 5) or 5)))
        ...
        reporter_name = bleach.clean(session.get('username', 'Unknown'), tags=[], strip=True)
        location = bleach.clean(data.get('location', '') or '', tags=[], strip=True)[:200]
        try:
            people_count = max(0, int(data.get('people_count', 0) or 0))
        except (ValueError, TypeError):
            people_count = 0
...
def add_responder(incident_id):
    """Assign a barangay official to respond to an incident."""
    incident = IncidentArchive.query.get_or_404(incident_id)
    data = request.json or {}
    name = bleach.clean(data.get('name', '').strip(), tags=[], strip=True)[:100]
    role = bleach.clean(data.get('role', 'Barangay Tanod').strip(), tags=[], strip=True)[:50]
    note = bleach.clean(data.get('note', '').strip(), tags=[], strip=True)[:300]
    if not name:
    ...
    # Auto-advance status to RESPONDING when first person responds
    ...
    return jsonify({
        ...
        'role': responder.responder_role,
        'assigned_at': responder.assigned_at.isoformat(),
        'note': responder.note or '',
        'incident_status': incident.incident_status,
    })


@app.route('/api/incidents/<int:incident_id>/responders/<int:responder_id>', methods=['DELETE'])
@login_required
def remove_responder(incident_id, responder_id):
    """Unassign a responder from an incident."""
    responder = IncidentResponder.query.filter_by(
        id=responder_id, incident_id=incident_id
    ).first_or_404()
    db.session.delete(responder)
    incident = IncidentArchive.query.get(incident_id)
    # Revert to OPEN if no responders remain
    if incident and not incident.responders:
...
def update_incident_status(incident_id):
    ...
    resolution_note = bleach.clean(data.get('resolution_note', '').strip(), tags=[], strip=True)[:1000]
    if resolution_note:
...
def _migrate_schema():
    """Add columns introduced after initial release without losing existing data."""
    from sqlalchemy import text
    new_cols = [
        'ALTER TABLE incident_archive ADD COLUMN title VARCHAR(200)',
        'ALTER TABLE incident_archive ADD COLUMN threat_level INTEGER DEFAULT 5',
        "ALTER TABLE incident_archive ADD COLUMN incident_status VARCHAR(20) DEFAULT 'OPEN'",
        'ALTER TABLE incident_archive ADD COLUMN reporter_name VARCHAR(100)',
        'ALTER TABLE incident_archive ADD COLUMN resolution_note TEXT',
        'ALTER TABLE incident_archive ADD COLUMN people_count INTEGER DEFAULT 0',
        'ALTER TABLE incident_archive ADD COLUMN location VARCHAR(200)',
    ]
    try:
...
# Always initialize DB regardless of how the app is started (python app.py OR flask run)
