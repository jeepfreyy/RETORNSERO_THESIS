...
class SentinelStream:
    ...
    def __init__(self, stream_id, source="video1.mp4", mask_path="mask_layer.png"):
        ...
        self.pause_saving = False  # Enabled: records 5-sec temp clips on anomalies
        ...
        self.clip_max_duration = 5        # 5-second temporary clips
        ...
        try:
            ...
            fallback = os.path.join(os.path.expanduser("~"), "Temp_Clips")
            print(f"[SentinelStream] Permission denied for '{self.temp_clips_dir}', using: '{fallback}'")
            self.temp_clips_dir = fallback
            os.makedirs(self.temp_clips_dir, exist_ok=True)
    ...
    def cleanup_temp_clips(self):
        """Delete all temporary clip files and thumbnails from disk."""
        if os.path.exists(self.temp_clips_dir):
            for fname in os.listdir(self.temp_clips_dir):
                fpath = os.path.join(self.temp_clips_dir, fname)
                try:
                    if os.path.isfile(fpath):
                        os.remove(fpath)
                except Exception as e:
                    print(f"[{self.stream_id}] Failed to delete temp file '{fname}': {e}")
        self.temp_clips.clear()
        print(f"[{self.stream_id}] All temporary clips cleaned up.")

    def stop(self):
        """Stop the stream and auto-delete all temporary clips."""
        self.running = False
        ...
        if self.clip_recording:
            self._stop_event_clip()
        # Auto-delete all temp clips from disk
        self.cleanup_temp_clips()
        print(f"[{self.stream_id}] Stream stopped and temp clips deleted.")